...
class AutoInstanceController: InstanceControllerProto {
    ...
    private func bootstrap() throws {
        ...
        Log.debug(message:
            "[AutoInstanceController] [\(name)] Bootstrap Status: \(allCells.count - missingCellIDs.count)/".cyan +
            "\(allCells.count) after \(Date().timeIntervalSince(start).rounded(toStringWithDecimals: 2))s".cyan
        )
        bootstrappLock.lock()
