...
public class Gym: JSONConvertibleObject, WebHookEvent, Hashable {
    ...
    init(id: String, lat: Double, lon: Double, name: String?, url: String?, guardPokemonId: UInt16?, enabled: Bool?,
         ...
         powerUpEndTimestamp: UInt32?, gymWeather: UInt8?) {
        self.id = id
        self.lat = lat
        self.lon = lon
        self.name = name
        ...
        if oldGym == nil {
            let sql = """
                INSERT INTO gym (
                    ...
                    power_up_end_timestamp, updated, first_seen_timestamp, gym_weather)
                VALUES (
                    ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
            ...
            let sql = """
                ...
                SET lat = ?, lon = ?, \(nameSQL) url = ?, guarding_pokemon_id = ?, last_modified_timestamp = ?,
                    ...
                    power_up_end_timestamp = ?, gym_weather = ?
                WHERE id = ?
            """
            self.updated = now
            _ = mysqlStmt.prepare(statement: sql)
    ...
    public static func getAll(mysql: MySQL?=nil, minLat: Double, maxLat: Double, minLon: Double, maxLon: Double,
        ...
        while let result = results.next() {
            ...
            let gymWeather = result[34] as? UInt8
            ...
            gyms.append(Gym(
                ...
                powerUpLevel: powerUpLevel, powerUpEndTimestamp: powerUpEndTimestamp, gymWeather: gymWeather))
        }
        return gyms
    ...
    public static func getWithIDs(mysql: MySQL?=nil, ids: [String]) throws -> [Gym] {
        ...
        SELECT id, lat, lon, name, url, guarding_pokemon_id, last_modified_timestamp, team_id,
               ...
               power_up_points, power_up_level, power_up_end_timestamp, gym_weather
        FROM gym
