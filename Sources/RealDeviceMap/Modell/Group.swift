...
struct Group {
    ...
    enum Perm: UInt32 {
        ...
        static var all: [Perm] = [
            ...
            .viewMapInvasion, .viewMapDevice, .viewMapSubmissionCells, .viewMapNests, .viewMapEventPokemon
        ]
    ...
    public static func getWithName(mysql: MySQL?=nil, name: String) throws -> Group? {
        ...
        let sql = """
            SELECT perm_view_map, perm_view_map_raid, perm_view_map_pokemon, perm_view_stats, perm_admin,
                   ...
                   perm_view_map_nests, perm_view_map_event_pokemon
            FROM `group`
            WHERE name = ?
        """
    ...
    public static func getAll(mysql: MySQL?=nil) throws -> [Group] {
        ...
        while let result = results.next() {
            ...
            let permViewMapNests = (result[17] as? UInt8)!.toBool()
            let permViewMapEventPokemon = (result[18] as? UInt8)!.toBool()
    ...
    public func save(mysql: MySQL?=nil, update: Bool!=true) throws {
        ...
        INSERT INTO `group` (
            ...
            perm_view_map_submission_cell, perm_view_map_nests, perm_view_map_event_pokemon
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        if update {
