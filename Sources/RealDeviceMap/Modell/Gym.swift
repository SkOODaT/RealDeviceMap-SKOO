...
class Gym: JSONConvertibleObject, WebHookEvent, Hashable {
    ...
    init(id: String, lat: Double, lon: Double, name: String?, url: String?, guardPokemonId: UInt16?, enabled: Bool?,
         ...
         raidIsExclusive: Bool?, cellId: UInt64?, totalCp: UInt32?, sponsorId: UInt16?, gymWeather: UInt8?) {
        self.id = id
        self.lat = lat
        self.lon = lon
        self.name = name
        ...
        if oldGym == nil {
            ...
            let sql = """
                INSERT INTO gym (
                    ...
                    sponsor_id, updated, first_seen_timestamp, gym_weather)
                VALUES (
                    ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
            ...
            let sql = """
                ...
                SET lat = ?, lon = ?, name = ?, url = ?, guarding_pokemon_id = ?, last_modified_timestamp = ?,
                    ...
                    raid_is_exclusive = ?, cell_id = ?, deleted = false, total_cp = ?, sponsor_id = ?, gym_weather = ?
                WHERE id = ?
            """
            _ = mysqlStmt.prepare(statement: sql)
        }
    ...
    public static func getAll(mysql: MySQL?=nil, minLat: Double, maxLat: Double, minLon: Double, maxLon: Double,
        ...
        while let result = results.next() {
            ...
            let gymWeather = result[28] as? UInt8
